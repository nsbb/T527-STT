...
|------|------|---------|--------|-----|----------|----------|-----|---------|
| [Conformer CTC](conformer_ctc/) | 한국어 | CNN + Attention (CTC) | **uint8 QAT** | **9.30%** | 3초 | 233ms | 0.11 | 102MB |
...
## Conformer CTC QAT — 한국어 CER 9.30% (SungBeom, 122.5M params)

PTQ(Post-Training Quantization)로는 CER 44%에 불과했던 Conformer CTC 모델을 QAT(Quantization-Aware Training)로 재학습하여 **CER 9.30%** 달성. 60+ 양자화 시도(10 아키텍처 × 21 방법) 중 유일하게 실용 수준에 도달한 결과.
...
- **핵심 발견**: val_loss 최소점(epoch 4) = NPU CER 100%, 최종 epoch 10 = NPU CER 9.30%. val_loss로 조기 종료하면 실패.
...
| 회의 (Meeting) | 13.66% |
| 강의 (Lecture) | 15.66% |
| 저품질 (Low quality) | 11.13% |
| 원거리 3m (Far-field) | 13.57% |
...
- **uint8 QAT로 한국어 CER 9.30% 달성** — Conformer CTC (122.5M params) + QAT (FakeQuantize + STE + MarginLoss)로 PTQ CER 44% → QAT CER 9.30%, 4.7배 개선
...
- **val_loss ≠ NPU CER** — QAT 학습 중 val_loss 최소 지점(epoch 4)이 NPU CER 100%, 최종 epoch 10이 CER 9.30%. 반드시 NPU 실측으로 검증 필요

### 모델별 상세 결과

각 모델 폴더의 README.md 참조:
- [Conformer CTC](conformer_ctc/): **CER 9.30%**, 233ms — **한국어 최고 성능 (QAT)** — [상세 결과](conformer_ctc/)
