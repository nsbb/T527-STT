...
# 121. Streaming Conformer CTC 구현 방법

현재 3초 chunk 슬라이딩 윈도우는 "가짜 스트리밍". 진짜 스트리밍은:

## 121.1 Chunk-wise Streaming

```
일반 Conformer:
  Attention: 모든 프레임 참조 (non-causal)
  Conv: 양방향 (future frames 참조)

Streaming Conformer:
  Attention: 현재 chunk + 과거 chunks만 참조 (causal mask)
  Conv: 왼쪽만 참조 (causal convolution)
```

**구현 변경:**
1. Self-Attention에 **causal mask** 추가 → future frames 차단
2. Depthwise Conv를 **causal conv**로 교체 (padding을 왼쪽에만)
3. 학습 시 **chunk mask**로 학습 → streaming/non-streaming 동시 지원 가능

## 121.2 T527에서의 Streaming

```
0.5초 chunk (50 frames) → NB 재변환 (input [1,80,50])
  → NPU 추론 ~40ms (3초 대비 1/6)
  → 0.5초마다 텍스트 업데이트
  → 사용자 체감 latency ~50ms
```

**하지만:** chunk가 작으면 context 부족 → CER 상승. 0.5초 chunk에서 CER 15~20% 예상 (3초 chunk CER 10% 대비).

**트레이드오프:** latency ↓ = accuracy ↓

| chunk | latency | CER 예상 |
|-------|---------|---------|
| 3초 (현재) | 233ms | 10.02% |
| 1.5초 | ~120ms | ~12% |
| 0.5초 | ~40ms | ~15~20% |
...
## 121.3 NeMo Streaming Conformer

NeMo에서 streaming 모드 지원:
```python
# config
model.encoder.streaming = True
model.encoder.chunk_size = 51  # 0.5초
model.encoder.left_context_size = 100  # 과거 1초
```

학습 후 streaming ONNX export → static chunk shape → Acuity NB → T527.
**미검증이지만 구현 가능할 것.**

참고: [SSCFormer: Chunk-wise Conformer for Streaming ASR](https://arxiv.org/abs/2211.11419), [Amazon: Dynamic Chunk Convolution](https://www.amazon.science/publications/dynamic-chunk-convolution-for-unified-streaming-and-non-streaming-conformer-asr)
...
이 문서는 **T527 NPU에서 한국어 STT를 배포하기 위해 필요한 모든 지식**을 담고 있다. 새로운 실험이 진행될 때마다 업데이트되어야 한다.
...
# 138. 모든 모델의 FP32 → uint8 CER 열화율

**FP32 CER 대비 uint8 CER의 열화 정도:**

| 모델 | FP32 CER | uint8 CER | **열화** | **열화율** |
|------|---------|-----------|---------|-----------|
| **SungBeom Conformer** | ~5% (추정) | **10.02%** | **+5%p** | **2x** |
| KoCitrinet | 8.44% | 44.44% | +36%p | **5.3x** |
| Wav2Vec2 EN | 9.74% | 17.52% | +7.78%p | **1.8x** |
| Wav2Vec2 KO base | 30.22% | 100.86% | +70.6%p | **3.3x** |
| Wav2Vec2 KO 80k | 9~18% | 92.83% | +75~84%p | **5~10x** |
| Zipformer | 16.2% | 100% | +83.8%p | **6.2x** |
| cwwojin Conformer | ~15% (추정) | 54.53% | +40%p | **3.6x** |

**관찰:**
- **SungBeom Conformer: 열화율 2x** — 가장 양자화에 강건
- **Wav2Vec2 EN: 열화율 1.8x** — 영어는 양자화에 강함
- **Wav2Vec2 KO: 열화율 5~10x** — 한국어 Transformer는 양자화에 극도로 취약
- **열화율 < 3x이면 실용적**, > 5x이면 사용 불가

---

# 139. SentencePiece BPE의 양자화 영향

BPE 토크나이저가 양자화에 미치는 간접적 영향:

## 139.1 BPE 토큰 길이와 CTC

```
BPE 2048 (SungBeom):
  "안녕하세요" → ["안녕", "하세요"] → 2 토큰
  CTC: 76 frames에서 2개 토큰 찾기 → 쉬움

음절 2048:
  "안녕하세요" → ["안", "녕", "하", "세", "요"] → 5 토큰
  CTC: 76 frames에서 5개 토큰 찾기 → 더 어려움

자모 56:
  "안녕하세요" → ["ㅇ","ㅏ","ㄴ","ㄴ","ㅕ","ㅇ","ㅎ","ㅏ","ㅅ","ㅔ","ㅇ","ㅛ"] → 12 토큰
  CTC: 76 frames에서 12개 토큰 찾기 → 가장 어려움
```

**BPE가 양자화에 유리한 이유:**
- 적은 토큰 수 → CTC alignment 실패 확률 낮음
- 의미 단위 토큰 → 유사 토큰 간 logit 차이가 큼 (margin 큼)
- 결과적으로 uint8 양자화 후에도 **argmax 보존 확률 높음**

---

# 140. 모든 실패 시도에 소요된 시간 상세

| 시도 | 시작 | 종료 | 소요 | 결과 | 교훈 |
|------|------|------|------|------|------|
| Wav2Vec2 EN 첫 시도 | 2/중순 | 3/15 | ~4주 | ✓ CER 17.52% | reverse_channel 버그 |
| Wav2Vec2 KO PTQ 21종 | 2/하순 | 3/18 | ~3주 | ✗ 전부 실패 | PTQ 한계 |
| Zipformer 양자화 | 3/19 | 3/20 | 2일 | ✗ CER 100% | 5868 노드 한계 |
| KoCitrinet int16 | 3/20 | 3/20 | 1일 | ✗ CER 330% | DFP < AA |
| 레이어 dump 분석 | 3/23 | 3/23 | 1일 | △ 인사이트 | L8-11 문제 |
| 영어→한국어 fine-tune | 3/23 | 3/23 | 1일 | △ WER 40.6% | 최초 성공 |
| QAT + margin loss | 3/24 | 3/24 | 1일 | △ margin 3x | 부족 |
| Vocab 분석 | 3/24 | 3/24 | 1일 | ✗ **틀림** | 확증 편향 |
| Split model 6종 | 3/24-25 | 3/25 | 2일 | ✗ 전부 실패 | encoder가 원인 |
| **Conformer NB 변환** | **3/25** | **3/25** | **1일** | **✓ CER 10.02%** | **아키텍처가 답** |

**총 소요: ~6주. 최적 경로로 했으면 1주면 끝남.**

---

# 141. T527 개발 환경 구축 가이드 (새 팀원용)
...
WSL Ubuntu 20.04
├── Docker (Acuity + NeMo)
├── Python 3.8+ (onnxruntime, numpy, soundfile)
├── onnxsim (pip install onnxsim)
├── adb (Windows SDK platform-tools 또는 WSL)
└── git (GitHub + Bitbucket)

Docker 이미지:
├── t527-npu:v1.2 (Acuity 6.12 + 6.21 + VeriSilicon SDK)
└── nvcr.io/nvidia/nemo:23.06 (NeMo + CUDA)
...
1. WSL + Docker 설치
2. `git clone git@github.com:nsbb/t527-stt.git`
3. `git clone git@github.com:nsbb/ai-sdk.git`
4. T527 보드 USB 연결 (`usbipd attach --wsl Ubuntu-20.04 --busid 1-5`)
5. adb 연결 확인 (`adb devices`)
6. **이 문서 읽기** (섹션 1-3, 21, 28)

---

# 142. 이 프로젝트에서 생성한 코드/스크립트 목록

| 파일 | 위치 | 용도 |
|------|------|------|
| `eval_wav2vec_cer.py` | `ai-sdk/.../wav2vec2_base_960h_5s/` | 영어 CER 측정 |
| `compute_cer_fair.py` | `ai-sdk/.../wav2vec2_ko_eager_op12/` | 한국어 CER (3초 truncation) |
| `compare_layers.py` | `wav2vec2/work/.../wksp_uint8_debug/` | FP32 vs uint8 레이어 비교 |
| `compare_ko_vs_en.py` | 같은 위치 | 한영 교차 비교 |
| `analyze_quantize.py` | 같은 위치 | .quantize 파라미터 분석 |
| `train_qat.py` | `ai-sdk/.../finetune/` | QAT 학습 |
| `train_qat_margin.py` | 같은 위치 | QAT + margin loss |
| `fix_onnx.py` | `ai-sdk/.../kr_sungbeom/` | Conformer ONNX Pad fix |
| `run_t527_test.py` | `ai-sdk/.../wav2vec2_base_korean/` | 80k 모델 디바이스 테스트 |
| `eval_split_model.py` | `ai-sdk/.../wav2vec2_base_960h_5s/` | Split model CER |

---

# 143. 이 문서에서 인용한 실측 데이터의 신뢰도

| 데이터 | 측정 방법 | 샘플 수 | 신뢰도 |
|--------|----------|---------|--------|
| SungBeom CER 10.02% | T527 vpm_run + Python decode | 100 | ★★★★★ |
| Wav2Vec2 EN CER 17.52% | T527 vpm_run + Python decode | 50 | ★★★★★ |
| KoCitrinet CER 44.44% | T527 Android app batch test | 330 | ★★★★★ |
| Wav2Vec2 KO CER 100.86% | T527 vpm_run + Python decode | 50 | ★★★★★ |
| Logit margin 실측 | Python (PyTorch forward) | 1~5 | ★★★ (소수 샘플) |
| 시뮬 vs 디바이스 31.5% | Acuity dump vs vpm_run | 1 | ★★ (단일 샘플) |
| 530개 레이어 비교 | Acuity 6.21 dump | 1 입력 | ★★★ |
| NPU 추론 시간 | vpm_run profile | 20 | ★★★★ |

**주의:** logit margin과 시뮬 vs 디바이스 비교는 **소수 샘플에서 측정**. 대규모 검증은 미완.
...
# 148. awnn API vs VIPLite API 차이

T527 NPU 추론에 사용 가능한 두 가지 API:

| | awnn (ai-sdk) | VIPLite (직접) |
|---|---|---|
| 래퍼 수준 | **고수준** (편의 API) | 저수준 (NPU 직접) |
| 초기화 | `awnn_init()` | `vip_init()` + `vip_create_network()` |
| 추론 | `awnn_run(ctx)` | `vip_run_network()` |
| 입출력 | `awnn_set_input_buffers()` | `vip_set_input()` + buffer 관리 |
| 스레드 안전 | O (내부 락) | △ (사용자 관리) |
| awaiasr_2 앱 | **사용 중** | — |
| bundle_app | — | VNN/OpenVX 사용 |
| vpm_run | — | VIPLite 직접 |

**권장:** Android 앱에서는 **awnn API** (간편). 성능 최적화나 멀티모델은 VIPLite 직접.
...
assets/models/
├── conformer/
│   ├── sungbeom_uint8_v1/        ← 현재 production
│   │   ├── network_binary.nb
│   │   ├── nbg_meta.json
│   │   └── vocab_correct.json
│   ├── sungbeom_uint8_v2/        ← 다음 버전 (aihub 학습)
│   │   └── ...
│   └── model_config.json         ← 현재 활성 버전 지정
├── kocitrinet/
│   └── ...
└── model_selector.json           ← 모델 전환 설정
```

**model_config.json:**
```json
{
  "active_model": "sungbeom_uint8_v1",
  "fallback_model": "kocitrinet",
  "input_length_sec": 3,
  "sliding_window_stride_frames": 250
}
```

---

# 150. 최종 확인: 이 문서에 빠진 것이 없는가
...
- [x] 의사결정 트리
- [x] 프로젝트 연혁
- [x] Kernel=31 ablation
- [x] SpecAugment 효과
- [x] Batch processing
- [x] Tiling strategy
- [x] Memory pool
- [x] NPU driver 호환성
- [x] awnn vs VIPLite API
- [x] 모델 버전 관리
- [x] 업데이트 기록
- [x] 검색 키워드
- [x] 읽기 가이드 (대상별)

**55항목 전부 확인 완료. 더 이상 추가할 내용 없음.**
