...
### 3.5 심층 분석: 영어/한국어 가중치 비교 (2026-03-17 신규)

영어(base-960h)와 한국어(base-korean) 모델의 실제 가중치를 직접 비교한 결과, **가중치는 거의 동일하지만 내부 활성화가 크게 다른** 원인을 특정함.

#### 가중치 비교 결과

##### CNN Feature Extractor — 거의 동일

| 레이어 | 최대 차이 | 평균 차이 | shape |
|--------|----------|----------|-------|
| conv_layers.0.conv.weight | 0.0146 | 0.0023 | (512, 1, 10) |
| conv_layers.1.conv.weight | 0.0268 | 0.0029 | (512, 512, 3) |
| conv_layers.2.conv.weight | 0.0303 | 0.0037 | (512, 512, 3) |
| conv_layers.3.conv.weight | 0.0299 | 0.0036 | (512, 512, 3) |
| conv_layers.4.conv.weight | 0.0332 | 0.0034 | (512, 512, 3) |
| conv_layers.5.conv.weight | 0.0287 | 0.0031 | (512, 512, 2) |
| conv_layers.6.conv.weight | 0.0228 | 0.0029 | (512, 512, 2) |
| feature_projection.weight | 0.2334 | 0.0207 | (768, 512) |
| **CNN 전체 평균 차이** | — | **0.0048** | 4,595,456 params |

> CNN 가중치 차이 0.48% — fine-tuning 과정에서 CNN이 거의 변하지 않음 (freeze 또는 매우 느린 학습률).

##### Transformer Attention — 영어보다 오히려 살짝 작음

| 레이어 | 가중치 | 영어 abs max | 한국어 abs max | 비율 |
|--------|--------|:----------:|:----------:|:----:|
| L0 | q_proj | 0.960 | 0.941 | 0.98x |
| L0 | k_proj | 1.287 | 1.275 | 0.99x |
| L0 | v_proj | 1.020 | 0.980 | 0.96x |
| L5 | q_proj | 0.951 | 0.870 | 0.92x |
| L5 | k_proj | 1.229 | 1.140 | 0.93x |
| L5 | v_proj | 0.411 | 0.364 | 0.89x |
| L11 | q_proj | 1.098 | 0.914 | 0.83x |
| L11 | k_proj | 1.275 | 1.050 | 0.82x |
| L11 | v_proj | 0.328 | 0.330 | 1.01x |

> 한국어 모델의 Transformer 가중치가 영어보다 **0.82~1.01배** — 가중치 자체는 오히려 좁음.

#### CNN→Transformer 경계 활성화 측정

ONNX 중간 출력(feature_projection)을 직접 추출하여 CNN 출력 범위 확인:

| 입력 | feature_projection 출력 범위 | std |
|------|:---------------------------:|:---:|
| 한국어 음성 (Zeroth-Korean 샘플 0) | [-6.72, 6.14] | 0.99 |
| 한국어 음성 (샘플 1) | [-6.47, 6.12] | 1.01 |
| 한국어 음성 (샘플 2) | [-8.73, 5.69] | 1.00 |
| 무음 (silence) | [-6.12, 4.14] | 0.91 |
| 화이트 노이즈 | [-4.76, 5.19] | 1.02 |
| 440Hz 순음 | [-4.40, 4.03] | 0.99 |

> CNN 출력은 LayerNorm을 거쳐 **std≈1.0으로 정규화**. 한국어/영어/무음 모두 비슷한 범위. **CNN은 병목이 아님.**

#### 핵심 발견: 문제는 Transformer 내부 dot product

```
[입력 오디오] → CNN(7 Conv1d) → LayerNorm → std≈1.0   ← 한/영 동일, 문제 없음
                                     ↓
                            Transformer Layer 0~11
                              Q = input × W_q      ← 가중치 거의 동일
                              K = input × W_k      ← 가중치 거의 동일
                              Score = Q @ K^T       ← 여기서 3-27배 벌어짐!
                              Softmax(Score)        ← 범위 27배 차이
                                     ↓
                            [Output logits]
```

**가중치는 0.82~1.01배 차이인데 활성화는 3~27배 차이.**

이유: Q@K^T는 dot product(768차원 내적). 가중치의 미세한 차이(0.82x)가 768개 차원에서 곱해져 누적되면:
- `0.82^2 = 0.67` (Q와 K 각각 0.82x이면 dot product는 0.67x)
- 하지만 실제로는 fine-tuning이 attention 패턴 자체를 변경
- 한국어는 교착어 특성으로 더 넓은 context 참조 → distributed attention → softmax 입력 범위 확대

> **결론: 같은 모델이라도 학습 언어에 따라 attention 패턴이 질적으로 달라지며, 이 차이가 uint8 양자화 호환성을 결정한다. Post-training 기법으로는 해결 불가능하며, QAT(학습 시 양자화 시뮬레이션)만이 유일한 경로.**
